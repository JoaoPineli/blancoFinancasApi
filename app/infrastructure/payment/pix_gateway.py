...
_T = TypeVar("_T")
_RETRYABLE_STATUSES: tuple[int, ...] = (408, 429, 500, 502, 503, 504)
...
async def _with_retry(
    ...
    *,
    ...
    Each attempt creates a fresh coroutine so that httpx.AsyncClient is
    re-created on retry (clients are not safe to reuse after a timeout).
    The idempotency key must be generated by the caller and captured in
    the factory closure so it is shared across all attempts.
    """
    for attempt in range(max_attempts):
        try:
            ...
            if attempt == max_attempts - 1:
                raise
            _log.warning(
                "MP request attempt %d/%d timed out — retrying in %.0fs",
                attempt + 1, max_attempts, base_delay_seconds * (2 ** attempt),
            )
            ...
            if attempt == max_attempts - 1:
                raise
            _log.warning(
                "MP request attempt %d/%d returned %d — retrying in %.0fs",
                attempt + 1, max_attempts, exc.response.status_code,
...
class PixGatewayAdapter:
    ...
    async def create_payment(
            ...
            async with httpx.AsyncClient() as http:
                response = await http.post(
                    ...
                    headers={
                        ...
                        # idempotency_key is captured from outer scope and reused
                        # across retry attempts so MP treats them as the same order.
        ...
        data = await _with_retry(_do_create)
    ...
    async def get_order(self, order_id: str) -> dict:
        ...
        return await _with_retry(_do_get)
